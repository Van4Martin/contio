...
export default function ManageSessionMotions() {
  ...
  useEffect(() => {
    adminService.getMotionsBySession(sessionId).then(setMotions).finally(() => setLoading(false))
  }, [sessionId])
  ...
  const handleCreate = async () => {
    ...
    setSaving(true)
    try {
      const m = await adminService.createMotion({ ...form, session_id: sessionId })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <main style={{ maxWidth: '700px', margin: '0 auto', padding: '40px 24px' }} className="fade-in">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '24px', fontWeight: 800 }}>Session Motions</h1>
        ...
        {motions.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', color: 'var(--text-muted)' }}>
            No motions yet for this session.
          </div>
        ) : (
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Add Motion">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} placeholder="Additional context..." style={{ padding: '11px 14px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'var(--font-body)', outline: 'none', resize: 'vertical' }} />
          </div>
