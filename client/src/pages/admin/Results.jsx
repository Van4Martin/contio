...
//   // Map: sessionTitle -> { [userId]: attendanceRecord }
//   const attendanceLookupBySession = {}
//   results.attendance.forEach(a => {
//     const session = resolveSessionTitle(a)
//     if (!attendanceLookupBySession[session]) attendanceLookupBySession[session] = {}
//     attendanceLookupBySession[session][a.user_id] = a
//   })

//   // For each session that has at least one real attendance record,
//   // build the FULL roster: all visible users, defaulting to Absent,
//   // overridden by their actual record if they checked in.
//   const attendanceBySession = {}
//   Object.keys(attendanceLookupBySession).forEach(sessionTitle => {
//     const lookup = attendanceLookupBySession[sessionTitle]

//     attendanceBySession[sessionTitle] = results.allUsers.map(u => {
//       const existing = lookup[u.id]
//       if (existing) {
//         // Real record — use it (preserves checked_in, checked_in_at, id, etc.)
//         return existing
//       }
//       // No record for this user in this session — synthetic "Absent" row
//       return {
//         id: `absent-${sessionTitle}-${u.id}`,
//         user_id: u.id,
//         checked_in: false,
//         checked_in_at: null,
...
import { useEffect, useState } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { ArrowLeft, Users, Vote, FileText, CheckCircle, XCircle, Minus, NotebookPen, Download, Eye, EyeOff } from 'lucide-react'
import Navbar from '../../components/layout/Navbar'
import Loader from '../../components/common/Loader'
import { adminService } from '../../services/adminService'
...
export default function Results() {
  ...
  // ✅ NEW: tracks which motion keys have comments revealed
  // key format: `${sectionTitle}__${motionTitle}`
  const [revealedComments, setRevealedComments] = useState({})

  const toggleComments = (key) => {
    setRevealedComments(prev => ({ ...prev, [key]: !prev[key] }))
  }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {Object.keys(electionResults).length === 0 ? (
              ...
              Object.entries(electionResults).map(([sectionTitle, categories]) => {
                ...
                return (
                  <div key={sectionTitle}>
                    <SectionLabel title={`Election Results — ${sectionTitle}`} />
                    {!isClosed ? (
                      <BlockedResults type="Election" />
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {Object.keys(motionResults).length === 0 ? (
              ...
              Object.entries(motionResults).map(([sectionTitle, motions]) => {
                ...
                return (
                  <div key={sectionTitle}>
                    <SectionLabel title={`Motion Results — ${sectionTitle}`}/>
                    {!isClosed ? (
                      <BlockedResults type="Motions" />
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        {Object.entries(motions).map(([motionTitle, data]) => {
                          const total    = data.yes + data.no + data.abstain + (data['juxta modum'] || 0)
                          const passed   = (data.yes + (data['juxta modum'] || 0)) > data.no
                          const commentKey = `${sectionTitle}__${motionTitle}`
                          const commentsRevealed = !!revealedComments[commentKey]
                                  ...
                                  <div>
                                    ...
                                    <button
                                      ...
                                      style={{
                                        ...
                                        background: commentsRevealed ? 'var(--bg-elevated)' : 'var(--bg-elevated)',
                                        border: `1px solid ${commentsRevealed ? 'var(--border-light)' : 'var(--border)'}`,
                                        borderRadius: commentsRevealed ? 'var(--radius-md) var(--radius-md) 0 0' : 'var(--radius-md)',
                                        color: commentsRevealed ? 'var(--text-primary)' : 'var(--text-muted)',
                                    ...
                                    >
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                                        {commentsRevealed
                                          ? <EyeOff size={13} style={{ color: 'var(--warning)' }} />
                                      ...
                                      <span style={{
                                        ...
                                        color: commentsRevealed ? 'var(--warning)' : 'var(--text-muted)',
                                      }}>
                                        {commentsRevealed ? 'Hide' : '🔒 Show'}
                                      </span>
                                    </button>

                                    {/* Comments — only visible when revealed */}
                                    {commentsRevealed && (
            ...
            {Object.keys(attendanceBySession).length === 0 ? (
              ...
              <>
                ...
                {Object.entries(attendanceBySession).map(([sessionTitle, records]) => {
                  ...
                  return (
                    <div key={sessionTitle}>
                      <SectionLabel title={`Attendance List — ${sessionTitle}`} />
                      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
