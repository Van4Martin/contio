...
//   // ── Results ──────────────────────────────────────────────────
//   async getFullResults(meetingId) {
//     const [sessions, attendance, peopleVotes, motionVotes, allUsers] = await Promise.all([
//       supabase
//         .from('sessions')
//         .select('id, title, order_index, election_closed_at, motions_closed_at')
//         .eq('meeting_id', meetingId)
...
//       // ✅ NEW: fetch all registered users (for the auto-absent list)
//       supabase
//         .from('users')
...
//     // ✅ NEW: exclude the hidden user entirely from the roster
//     const HIDDEN_EMAIL = 'win4martin@gmail.com'
//     const visibleUsers = (allUsers.data || []).filter(u => u.email !== HIDDEN_EMAIL)
  ...
  async getReportData() {
    const [meetings, users, attendance, peopleVotes, motionVotes] = await Promise.all([
      supabase
        .from('meetings')
        .select('*')
        .eq('is_archived', false)   // exclude archived from reports
